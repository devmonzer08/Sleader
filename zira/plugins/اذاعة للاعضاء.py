...
async def malath(event):
    ...
    zzz = await edit_or_reply(event, YYY4YI_BEST_SOURCE, link_preview=False)
    total = 0
    success = 0
    async for usr in event.client.iter_participants(event.chat_id):
        ...
        if zilzal.text:
            ...
            try:
                ...
                return
    ZELZAL_BEST_DEV = f"[ᯓ 𝗟𝗘𝗔𝗗𝗘𝗥 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة خـاص 🚹](t.me/YYY4YI) .\n\n**⎉╎تمت الاذاعـه لـ اعضـاء الكـروب .. بنجـاح  ✅**\n**⎉╎عـدد {success} عضـو**"
...
async def malath(event):
    ...
    users = zelzal.split(" ")
    zzz = await edit_or_reply(event, YYY4YI_PRO_SOURCE, link_preview=False)
    total = 0
    success = 0
    user_entity = None
    for user in users:
        ...
        if zilzal.text:
            ...
            try:
                ...
                zzz.edit(f"خطأ في إرسال الرسالة إلى {user_entity.id}: {str(e)}")
    ZELZAL_BEST_DEV = f"[ᯓ 𝗟𝗘𝗔𝗗𝗘𝗥 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة زاجـل 🕊](t.me/YYY4YI) .\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**⎉╎تمت الاذاعـه .. بنجـاح  ✅**\n**⎉╎عـدد {success} أشخـاص**"
    await zzz.edit(ZELZAL_BEST_DEV, link_preview=False)
